// 1. DATABASE DATA MENU (ID sudah dirapikan agar tidak tabrakan & ditambah status tersedia)
const daftarMenu = [
    // MINUMAN
...
// 2. STATE DATABASE STATUS MEJA & AUDIO SUARA SUKSES
...
// 🔊 Inisialisasi file suara MP3 (Pastikan file 'sukses.mp3' ada di folder yang sama dengan file ini)
...
// 4. FUNGSI RENDER MENU (SUDAH DUKUNG FITUR HABIS)
function tampilkanMenu() {
    ...
    daftarMenu.forEach(item => {
        ...
        // Ditambahkan oncontextmenu agar kasir bisa klik kanan / tahan lama di HP untuk merubah status ketersediaan barang
        const htmlCard = `
            <div class="card-menu${kelasTambahanCard}" 
                ...
                <small style="color: #ccc; font-size: 0.6rem; position: absolute; top: 4px; right: 6px;">Tahan / Klik Kanan</small>
                <h3>${item.nama}</h3>
                <p class="harga">${labelHargaHTML}</p>
                ${tombolHTML}
            </div>
        `;
...
// FUNGSI KHUSUS UNTUK BOLAK-BALIK STATUS HABIS <=> ADA
function beralihStatusMenu(id) {
    const music = daftarMenu.find(item => item.id === id);
    if (music) {
        music.tersedia = !music.tersedia;
        
        // Kalau barang diset habis, hapus otomatis dari keranjang belanja meja yang sedang aktif (jika ada)
        if (!music.tersedia) {
            let keranjangSekarang = dataMeja[mejaAktif].keranjang;
...
function tambahItem(id) {
    ...
    if (!produk.tersedia) return; // Kunci biar tidak bisa ditambah kalau habis
...
// 10. PROSES TRANSAKSI SELESAI (DENGAN SOUND EFEK MP3)
function prosesTransaksi() {
    if (metodeSekarang === 'tunai') {
        ...
        // 🎵 Putar sound MP3 sukses saat pembayaran tunai berhasil
        suaraSukses.play();

        const kembalian = nominalUang - totalTagihanGlobal;
        alert(`TRANSAKSI TUNAI MEJA ${mejaAktif} LUNAS!\n\nKembalian: Rp ${kembalian.toLocaleString('id-ID')}\n\nMeja otomatis dikosongkan.`);
    } else {
        // 🎵 Putar sound MP3 sukses saat pembayaran QR Code berhasil
        suaraSukses.play();
