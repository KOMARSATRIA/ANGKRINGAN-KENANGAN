...
// ==========================================================================
// INTEGRASI QRIS DINAMIS ASLI ANGKRINGAN KENANGAN (OTOMATIS NOMINAL)
// ==========================================================================
function bukaModalPembayaran() {
    ...
    // --- PROSES GENERATE QRIS DINAMIS ---
    const qrImageElement = document.querySelector('.qr-image');
    if (qrImageElement) {
        // Data QRIS statis asli punyamu (tanpa CRC / 4 digit terakhir "FE28")
        let qrisBase = "00020101021126570011ID.DANA.WWW011893600915303262370202090326237020303UMI51440014ID.CO.QRIS.WWW0215ID10265320725880303UMI5204581353033605802ID5919Angkringan kenangan6015Kota Pekalongan610551135";
        
        // Format nominal transaksi menjadi teks (contoh: 15000)
        let nominalString = totalTagihanGlobal.toString();
        
        // Menyusun tag 54 (Kuantitas/Nilai Transaksi) sesuai standar EMVCo QRIS
        // "54" + 2 digit panjang karakter nominal + nominal uangnya
        let tagNominal = "54" + String(nominalString.length).padStart(2, '0') + nominalString;
        
        // Gabungkan data dasar QRIS dengan nominal uang baru
        let qrisLengkapTanpaCRC = qrisBase + tagNominal + "6304";
        
        // Fungsi pembantu hitung CRC16 otomatis agar QR Code valid dan bisa di-scan m-banking/e-wallet
        let crcHasil = hitungCRC16(qrisLengkapTanpaCRC);
        let qrisFinalDinamis = qrisLengkapTanpaCRC + crcHasil;
        
        // Ubah gambar QR di modal menggunakan QR Server API gratis berdasarkan data teks dinamis yang baru
        qrImageElement.src = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(qrisFinalDinamis)}`;
    }
    // -------------------------------------
...
// ==========================================================================
// FUNGSI TAMBAHAN: HITUNG CRC16 AUTOMATIC (WAJIB ADA SUPAYA QRIS TIDAK ERROR)
// ==========================================================================
function hitungCRC16(str) {
    let crc = 0xFFFF;
    for (let c = 0; c < str.length; c++) {
        let b = str.charCodeAt(c);
        for (let i = 0; i < 8; i++) {
            let bit = ((b >> (7 - i) & 1) == 1);
            let c15 = ((crc >> 15 & 1) == 1);
            crc <<= 1;
